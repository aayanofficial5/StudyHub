...
const ContactUsForm = ({ title, paragraph }) => {
  const {
    register,
    reset,
    handleSubmit,
  ...
  return (
    <div className="max-w-xl min-w-xs border-1 rounded-2xl backdrop-blur bg-white/10">
      <div className="flex flex-col items-center">
        <div className={`w-[79%] font-bold text-3xl text-center mt-5`}>
          {title}
        </div>
        <div className={`w-[70%] mt-5 text-lg text-gray-400 text-center`}>
          {paragraph}
        </div>
      </div>
      <form
        onSubmit={handleSubmit(submitContactForm)}
        className="flex flex-col gap-4 w-full max-w-3xl mx-auto p-5 mt-8"
      >
        ...
        <button
          type="submit"
          className="w-full flex justify-center py-2 bg-gradient-to-tr from-blue-400 via-blue-700 to-blue-400 rounded-md hover:scale-95 hover:text-black transition-all duration-400 ease-in-out mt-5 cursor-pointer font-semibold text-base border-3 border-blue-500 gap-4"
        >
