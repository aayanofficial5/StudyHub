...
const CourseBuilderForm = () => {
  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 text-white w-full" >
        <div>
      ...
      <div className="flex flex-row items-end gap-5">
        ...
        >{editSectionName?<FaEdit/>:<IoMdAddCircle size={25}/>}</CTAButton>
        {editSectionName&&(<button className="underline text-sm cursor-pointer" onClick={handleCancelEdit}>
          Cancel Edit
        </button>)}
      </div>
      </form>
      ...
      <div className="flex justify-end gap-x-3">
        ...
        <CTAButton active={true} text="Next" arrow={true} 
        action={goNext}/>
      </div>
    </div>
  );
